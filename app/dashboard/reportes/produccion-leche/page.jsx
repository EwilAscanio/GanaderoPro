...
function ReportDocument({ registros, fecha_desde, fecha_hasta }) {
  ...
  const totalLitros = registros.reduce((sum, r) => sum + Number(r.litros_lec), 0);
  ...
  return (
    <Document>
      <Page size="A4" orientation="landscape" style={pdfStyles.page}>
        ...
        <Text style={pdfStyles.title}>Reporte de Producción de Leche</Text>
        <Text style={pdfStyles.subtitle}>
          {`Período: ${fecha_desde || "Inicio"} - ${fecha_hasta || "Fin"}`} | {`Total registros: ${registros.length}`} | {`Total litros: ${totalLitros.toFixed(2)}`}
        </Text>

        <View style={pdfStyles.table}>
          <View style={pdfStyles.headerRow} wrap={false}>
            <Text style={pdfStyles.cellId}>ID</Text>
            <Text style={pdfStyles.cellCodigo}>Código Animal</Text>
            <Text style={pdfStyles.cellNombre}>Nombre</Text>
            <Text style={pdfStyles.cellFecha}>Fecha</Text>
            <Text style={pdfStyles.cellLitros}>Litros</Text>
          </View>
          {registros.map((r) => (
            <View key={r.id_lec} style={pdfStyles.row}>
              <Text style={pdfStyles.cellId}>{r.id_lec}</Text>
              <Text style={pdfStyles.cellCodigo}>{r.codigo_ani}</Text>
              <Text style={pdfStyles.cellNombre}>{r.nombre_ani}</Text>
              <Text style={pdfStyles.cellFecha}>{formatDate(r.fecha_lec)}</Text>
              <Text style={pdfStyles.cellLitros}>{Number(r.litros_lec).toFixed(2)}</Text>
            </View>
          ))}
        </View>
...
function ReporteProduccionLecheContent() {
  ...
  useEffect(() => {
    ...
    axios.get("/api/reportes/produccion-leche", { params })
  ...
  const handleDownloadPDF = async () => {
    ...
    try {
      ...
      a.href = url;
      a.download = `Reporte_ProduccionLeche_${new Date().toISOString().slice(0, 10)}.pdf`;
      a.click();
  ...
  const totalLitros = registros ? registros.reduce((sum, r) => sum + Number(r.litros_lec), 0) : 0;
          ...
          <div className="flex items-center justify-between p-4 border-b" style={{ borderColor: "var(--border)" }}>
            <div>
              ...
              <p className="text-xs mt-0.5" style={{ color: "var(--text-muted)" }}>
                {fecha_desde || "Inicio"} - {fecha_hasta || "Fin"} | Total: {totalLitros.toFixed(2)} litros
              </p>
            </div>
            {registros?.length > 0 && (
              <button
              ...
              <table className="w-full text-sm">
                <thead>
                  <tr style={{ background: "var(--bg-secondary)" }}>
                    ...
                    <th className="text-left px-4 py-3 font-medium sticky top-0 hidden sm:table-cell" style={{ color: "var(--text-muted)", background: "var(--bg-secondary)" }}>Fecha</th>
                    <th className="text-left px-4 py-3 font-medium sticky top-0" style={{ color: "var(--text-muted)", background: "var(--bg-secondary)" }}>Litros</th>
                  </tr>
                </thead>
                <tbody>
                  {registros.map((r, i) => (
                    ...
                    >
                      ...
                      <td className="px-4 py-3 hidden sm:table-cell" style={{ color: "var(--text-secondary)" }}>
                        {formatDate(r.fecha_lec)}
                      </td>
                      <td className="px-4 py-3 font-semibold" style={{ color: "var(--text-primary)" }}>
                        {Number(r.litros_lec).toFixed(2)}
                      </td>
                    </tr>
                  ))}
                </tbody>
...
export default function ReporteProduccionLechePage() {
  return (
    <Suspense fallback={
      <div className="flex justify-center py-20">
        <div
