...
const pdfStyles = StyleSheet.create({
  ...
  cellNombre: { width: "20%" },
  cellFecha: { width: "17%" },
  cellEmbarazada: { width: "15%" },
  cellGestion: { width: "13%" },
  cellCreated: { width: "12%" },
...
function ReportDocument({ registros, fecha_desde, fecha_hasta }) {
  ...
  const totalEmbarazadas = registros.filter((r) => r.animalembarazado_pal).length;
  ...
  return (
    <Document>
      <Page size="A4" orientation="landscape" style={pdfStyles.page}>
        ...
        <Text style={pdfStyles.title}>Reporte de Palpaciones</Text>
        <Text style={pdfStyles.subtitle}>
          {`Período: ${fecha_desde || "Inicio"} - ${fecha_hasta || "Fin"}`} | {`Total: ${registros.length}`} | {`Embarazadas: ${totalEmbarazadas}`}
        </Text>
        ...
        <View style={pdfStyles.table}>
          <View style={pdfStyles.headerRow} wrap={false}>
            ...
            <Text style={pdfStyles.cellFecha}>Fecha Palp.</Text>
            <Text style={pdfStyles.cellEmbarazada}>Embarazada</Text>
            <Text style={pdfStyles.cellGestion}>Gest.(días)</Text>
            <Text style={pdfStyles.cellCreated}>Registrado</Text>
          </View>
          {registros.map((r) => (
            <View key={r.id_pal} style={pdfStyles.row}>
              <Text style={pdfStyles.cellId}>{r.id_pal}</Text>
              <Text style={pdfStyles.cellCodigo}>{r.codigo_ani}</Text>
              <Text style={pdfStyles.cellNombre}>{r.nombre_ani}</Text>
              <Text style={pdfStyles.cellFecha}>{formatDate(r.fecha_pal)}</Text>
              <Text style={pdfStyles.cellEmbarazada}>{r.animalembarazado_pal ? "Sí" : "No"}</Text>
              <Text style={pdfStyles.cellGestion}>{r.tiempogestacion_pal > 0 ? r.tiempogestacion_pal : ""}</Text>
              <Text style={pdfStyles.cellCreated}>{formatDate(r.created_at)}</Text>
            </View>
          ))}
        </View>

        <Text style={pdfStyles.totalRow}>{`Total: ${registros.length} | Embarazadas: ${totalEmbarazadas}`}</Text>
...
function ReportePalpacionesContent() {
  ...
  useEffect(() => {
    ...
    axios.get("/api/reportes/palpaciones", { params })
  ...
  const handleDownloadPDF = async () => {
    ...
    try {
      ...
      a.href = url;
      a.download = `Reporte_Palpaciones_${new Date().toISOString().slice(0, 10)}.pdf`;
      a.click();
  ...
  const totalEmbarazadas = registros
    ? registros.filter((r) => r.animalembarazado_pal).length
    : 0;
  ...
  return (
    <div className="space-y-6 animate-fade-in-up">
      <div className="flex items-center gap-4">
        ...
        <div>
          <h1 className="text-2xl lg:text-3xl font-bold gradient-text">Palpaciones</h1>
        ...
        >
          <div className="flex items-center justify-between p-4 border-b" style={{ borderColor: "var(--border)" }}>
            <div>
              ...
              <p className="text-xs mt-0.5" style={{ color: "var(--text-muted)" }}>
                {fecha_desde || "Inicio"} - {fecha_hasta || "Fin"} | Embarazadas: {totalEmbarazadas}
              </p>
            </div>
            {registros?.length > 0 && (
              <button
          ...
          {!registros || registros.length === 0 ? (
            <p className="text-center py-10 text-sm" style={{ color: "var(--text-muted)" }}>
              No se encontraron registros de palpación con los filtros seleccionados.
            </p>
          ) : (
              ...
              <table className="w-full text-sm">
                <thead>
                  <tr style={{ background: "var(--bg-secondary)" }}>
                    ...
                    <th className="text-left px-4 py-3 font-medium sticky top-0" style={{ color: "var(--text-muted)", background: "var(--bg-secondary)" }}>Embarazada</th>
                    <th className="text-left px-4 py-3 font-medium sticky top-0 hidden sm:table-cell" style={{ color: "var(--text-muted)", background: "var(--bg-secondary)" }}>Gest.(días)</th>
                    <th className="text-left px-4 py-3 font-medium sticky top-0 hidden md:table-cell" style={{ color: "var(--text-muted)", background: "var(--bg-secondary)" }}>Registrado</th>
                  </tr>
                </thead>
                <tbody>
                  {registros.map((r, i) => (
                    ...
                    >
                      ...
                      <td className="px-4 py-3" style={{ color: "var(--text-secondary)" }}>
                        {formatDate(r.fecha_pal)}
                      </td>
                      <td className="px-4 py-3">
                        <span
                          className={`inline-flex px-2 py-0.5 rounded text-xs font-medium ${
                            r.animalembarazado_pal
                              ? "text-green-600 bg-green-50"
                              : "text-gray-500 bg-gray-100"
                          }`}
                        >
                          {r.animalembarazado_pal ? "Sí" : "No"}
                        </span>
                      </td>
                      <td className="px-4 py-3 hidden sm:table-cell" style={{ color: "var(--text-secondary)" }}>
                        {r.tiempogestacion_pal > 0 ? r.tiempogestacion_pal : ""}
                      </td>
...
export default function ReportePalpacionesPage() {
  return (
    <Suspense fallback={
      <div className="flex justify-center py-20">
        <div
