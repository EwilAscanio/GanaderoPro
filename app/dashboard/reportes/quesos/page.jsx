...
function ReportDocument({ registros, fecha_desde, fecha_hasta }) {
  ...
  const totalKg = registros.reduce((sum, r) => sum + Number(r.kg_que), 0);
  ...
  return (
    <Document>
      <Page size="A4" orientation="landscape" style={pdfStyles.page}>
        ...
        <Text style={pdfStyles.title}>Reporte de Quesos</Text>
        ...
        <View style={pdfStyles.table}>
          ...
          {registros.map((r) => (
            <View key={r.id_que} style={pdfStyles.row}>
              <Text style={pdfStyles.cellId}>{r.id_que}</Text>
              <Text style={pdfStyles.cellFecha}>{formatDate(r.fecha_que)}</Text>
              <Text style={pdfStyles.cellKg}>{Number(r.kg_que).toFixed(2)}</Text>
            </View>
          ))}
        </View>
...
function ReporteQuesosContent() {
  ...
  useEffect(() => {
    ...
    axios.get("/api/reportes/quesos", { params })
  ...
  const handleDownloadPDF = async () => {
    ...
    try {
      ...
      a.href = url;
      a.download = `Reporte_Quesos_${new Date().toISOString().slice(0, 10)}.pdf`;
      a.click();
  ...
  const totalKg = registros ? registros.reduce((sum, r) => sum + Number(r.kg_que), 0) : 0;
                  ...
                  {registros.map((r, i) => (
                    ...
                    >
                      ...
                      <td className="px-4 py-3" style={{ color: "var(--text-secondary)" }}>
                        {formatDate(r.fecha_que)}
                      </td>
                      <td className="px-4 py-3 font-semibold" style={{ color: "var(--text-primary)" }}>
                        {Number(r.kg_que).toFixed(2)}
                      </td>
                    </tr>
                  ))}
                </tbody>
...
export default function ReporteQuesosPage() {
  return (
    <Suspense fallback={
      <div className="flex justify-center py-20">
        <div
