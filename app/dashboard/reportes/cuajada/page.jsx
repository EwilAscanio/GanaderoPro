...
function ReportDocument({ registros, fecha_desde, fecha_hasta }) {
  ...
  const totalKg = registros.reduce((sum, r) => sum + Number(r.kg_cua), 0);
  ...
  return (
    <Document>
      <Page size="A4" orientation="landscape" style={pdfStyles.page}>
        ...
        <Text style={pdfStyles.title}>Reporte de Cuajada</Text>
        <Text style={pdfStyles.subtitle}>
          {`Período: ${fecha_desde || "Inicio"} - ${fecha_hasta || "Fin"}`} | {`Total registros: ${registros.length}`} | {`Total Kg: ${totalKg.toFixed(2)}`}
        </Text>
        ...
        <View style={pdfStyles.table}>
          <View style={pdfStyles.headerRow} wrap={false}>
            ...
            <Text style={pdfStyles.cellKg}>Kg</Text>
          </View>
          {registros.map((r) => (
            <View key={r.id_cua} style={pdfStyles.row}>
              <Text style={pdfStyles.cellId}>{r.id_cua}</Text>
              <Text style={pdfStyles.cellFecha}>{formatDate(r.fecha_cua)}</Text>
              <Text style={pdfStyles.cellKg}>{Number(r.kg_cua).toFixed(2)}</Text>
            </View>
          ))}
        </View>

        <Text style={pdfStyles.totalRow}>{`Total Kg: ${totalKg.toFixed(2)}`}</Text>
...
function ReporteCuajadaContent() {
  ...
  useEffect(() => {
    ...
    axios.get("/api/reportes/cuajada", { params })
  ...
  const handleDownloadPDF = async () => {
    ...
    try {
      ...
      a.href = url;
      a.download = `Reporte_Cuajada_${new Date().toISOString().slice(0, 10)}.pdf`;
      a.click();
  ...
  const totalKg = registros ? registros.reduce((sum, r) => sum + Number(r.kg_cua), 0) : 0;
          ...
          <div className="flex items-center justify-between p-4 border-b" style={{ borderColor: "var(--border)" }}>
            <div>
              ...
              <p className="text-xs mt-0.5" style={{ color: "var(--text-muted)" }}>
                {fecha_desde || "Inicio"} - {fecha_hasta || "Fin"} | Total: {totalKg.toFixed(2)} kg
              </p>
            </div>
            {registros?.length > 0 && (
              <button
              ...
              <table className="w-full text-sm">
                <thead>
                  <tr style={{ background: "var(--bg-secondary)" }}>
                    ...
                    <th className="text-left px-4 py-3 font-medium sticky top-0" style={{ color: "var(--text-muted)", background: "var(--bg-secondary)" }}>Kg</th>
                  </tr>
                </thead>
                <tbody>
                  {registros.map((r, i) => (
                    ...
                    >
                      ...
                      <td className="px-4 py-3" style={{ color: "var(--text-secondary)" }}>
                        {formatDate(r.fecha_cua)}
                      </td>
                      <td className="px-4 py-3 font-semibold" style={{ color: "var(--text-primary)" }}>
                        {Number(r.kg_cua).toFixed(2)}
                      </td>
                    </tr>
                  ))}
                </tbody>
...
export default function ReporteCuajadaPage() {
  return (
    <Suspense fallback={
      <div className="flex justify-center py-20">
        <div
